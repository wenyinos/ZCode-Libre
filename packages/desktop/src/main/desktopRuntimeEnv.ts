...
//
// 应用名同时决定 Electron userData 目录：本分支使用独立的 ZCode-Libre 目录，
// 因此可与官方客户端同时运行（单实例锁按数据目录生效），也避免两边版本分叉后
// 共用 Electron 缓存与存储格式。会话记录、模型设置与凭据存放在 .zcode 数据根下
// （见 packages/services/src/paths.ts），那部分仍与上游共享，不受此处影响。
